...
export const electronicsModule: VMModule = {
  id: "electronics",
  name: "Electronics Studio",
  ...
  registerComponents(world) {
    world
      ...
      .registerComponent(GazeTarget);
  },

  registerSystems(world) {
    world
      .registerSystem(SnapSystem)
      .registerSystem(SnapFailFeedbackSystem)
      .registerSystem(HoverPreviewSystem)
      .registerSystem(CircuitEvalSystem)
      .registerSystem(HudSystem)
      .registerSystem(HypothesisCardSystem)
      .registerSystem(AttemptHistorySystem)
      .registerSystem(IdleCharacterSystem)
      .registerSystem(GazeSystem, { priority: 35 });
  },

  async setup({ world, condition }: ModuleContext) {
    ...
    const board = createBreadboard(world, {
      position: [-0.2, 0.86, -1.05],
    }).addComponent(GazeTarget, { name: "breadboard" });
    ...
    guidesGroup.visible = condition === "DI";
    ...
    const hudEntity = world
      ...
      .addComponent(GazeTarget, { name: "hud" });

    world
      .createTransformEntity()
      .addComponent(PanelUI, {
        config: "./ui/hypothesis-card.json",
        maxHeight: 0.18,
        maxWidth: 0.32,
      })
      .addComponent(Follower, {
        target: world.player.head,
        offsetPosition: [0.28, -0.3, -0.6],
        behavior: FollowBehavior.PivotY,
        maxAngle: 30,
        tolerance: 0.4,
        speed: 1,
    ...
    const Y = 0.94;
    spawnLed(world, [0.1, Y, -1.22], 0xff3030, "led_red");
    spawnLed(world, [0.2, Y, -1.22], 0x30ff30, "led_green");
    spawnResistor(world, [0.4, Y, -1.22], 0xc8a060, "resistor_gold");
    spawnBattery(world, [0.62, Y, -1.22], "battery");
    spawnResistor(world, [0.1, Y, -1.05], 0xa07050, "resistor_brown");
    spawnWire(world, [0.3, Y, -1.05], 0.048, 0xc62828, "wire_red_short");
    spawnWire(world, [0.55, Y, -1.05], 0.12, 0x1565c0, "wire_blue_med");
    spawnWire(world, [0.4, Y, -0.94], 0.24, 0xfdd835, "wire_yellow_long");
    ...
    world
      .createTransformEntity(robotWrapper)
      .addComponent(IdleCharacter, {
        centerX: -1.8,
        centerY: 0,
        centerZ: -1.0,
        radius: 0.4,
      ...
      .addComponent(GazeTarget, { name: "robot" });
    ...
    if (import.meta.env.DEV) {
      ...
      (
        window as Window & { __VM_SHOW_SNAP_FAIL?: typeof showSnapFailMarkers }
      ).__VM_SHOW_SNAP_FAIL = showSnapFailMarkers;
    }
  },
};
